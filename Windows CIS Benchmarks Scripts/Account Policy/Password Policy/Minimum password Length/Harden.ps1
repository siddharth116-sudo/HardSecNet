...
.DESCRIPTION
This script sets the MinimumPasswordLength to 14 using secedit.exe to enforce 
stronger local account passwords, minimizing the attack surface from brute-force attempts.
...
$CIS_ID = "1.1.4 (L1)"
$CIS_NAME = "Ensure 'Minimum password length' is set to '14 or more character(s)' (Automated)"
$REQUIRED_VALUE = 14 # 14 characters or more
$SETTING_KEY = "MinimumPasswordLength"
...
try {
    ...
    $TempCfgPath = Join-Path $env:TEMP "MinLength_Config.inf"
    $TempSdbPath = Join-Path $env:TEMP "MinLength_Database.sdb"

    # 2. Create Minimal .inf Configuration File
    # The [System Access] section is used for MinimumPasswordLength
    ...
    Write-Host "Setting $SETTING_NAME to $REQUIRED_VALUE characters..." -ForegroundColor Yellow
    ...
    # Extract the MinimumPasswordLength value using regex
    ...
    $CurrentStatusText = if ($Status -eq "COMPLIANT") { "Value is >= $REQUIRED_VALUE characters" } else { "Value is NOT >= $REQUIRED_VALUE characters" }

    Write-Host $StatusSeparator -ForegroundColor DarkYellow
    Write-Host "SUCCESS: Hardening applied." -ForegroundColor Green
    Write-Host $StatusSeparator -ForegroundColor DarkYellow
    
    # TUI output formatted to match screenshot
    Write-Host ("{0,-30} : {1} characters" -f "Required CIS Minimum", $REQUIRED_VALUE) -ForegroundColor Yellow
    Write-Host ("{0,-30} : {1} characters" -f "Current Applied Value", $CurrentAppliedValue) -ForegroundColor Yellow
